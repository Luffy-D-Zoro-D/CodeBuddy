...
//#region node_modules/.nitro/vite/services/ssr/assets/start-CHP8Ch0j.js
...
var apiProtectionMiddleware = createMiddleware().server(async ({ next, request }) => {
	const origin = request.headers.get("origin");
	const referer = request.headers.get("referer");
	const secFetchSite = request.headers.get("sec-fetch-site");
	if (request.method !== "GET" && !origin && !referer && !secFetchSite) return new Response("Direct API access via tools like Postman is forbidden.", { status: 403 });
	return await next();
});
